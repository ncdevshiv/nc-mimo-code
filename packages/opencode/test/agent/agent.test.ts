...
test("plan agent denies edits except .nc-mimo-code/plans/*", async () => {
  await using tmp = await tmpdir()
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      ...
      expect(Permission.evaluate("edit", ".nc-mimo-code/plans/foo.md", plan!.permission).action).toBe("allow")
    },
  })
})
