...
function actorFlagError(line: number, detail: string) {
  return Effect.fail({ kind: "flag" as const, line, detail: `actor: ${detail}` })
}

const mapActorVerb = Effect.fn("mapActorVerb")(function* (verb: string | undefined, args: string[], line: number) {
  switch (verb) {
    case "run": {
      const ex = extractFlags(args, ["model", "task", "actor", "timeout", "command", "context", "output-schema"])
      if (ex.error) return yield* actorFlagError(line, ex.error)
      const { flags, rest } = ex
    ...
    case "spawn": {
      const ex = extractFlags(args, ["model", "task", "actor", "command", "context", "output-schema"])
      if (ex.error) return yield* actorFlagError(line, ex.error)
      const { flags, rest } = ex
    ...
    case "wait": {
      const ex = extractFlags(args, ["timeout"])
      if (ex.error) return yield* actorFlagError(line, ex.error)
      const { flags, rest } = ex
    ...
    case "send": {
      const ex = extractFlags(args, ["session", "type"])
      if (ex.error) return yield* actorFlagError(line, ex.error)
      const { flags, rest } = ex
      if (rest.length !== 2)
    ...
    default: {
      const suggestion = suggestVerb(verb ?? "", KNOWN_ACTOR_VERBS)
      const detail =
