        ...
        return Effect.gen(function* () {
          yield* Effect.try({
            try: () => toolInfo.parameters.parse(args),
              ...
              // ZodError.issues (and arbitrary `error` shapes from custom params
              // parsers) can run to many KB; truncate so a single bad tool call
              // doesn't blow the model's context with the error string.
