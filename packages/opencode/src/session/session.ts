import { Slug } from "@nc-mimo-code/shared/util/slug"
...
export const getUsage = (input: { model: Provider.Model; usage: LanguageModelUsage; metadata?: ProviderMetadata }) => {
  ...
  return {
    ...
    // Reasoning tokens are charged at the same rate as output
    // tokens — the upstream `models.dev` schema does not yet have
    // a separate reasoning-tokens field, and most providers that
    // expose a reasoning-token count (anthropic, openai) price
    // them the same as output. When models.dev adds a `reasoning`
    // price, swap this to `costInfo?.reasoning ?? costInfo?.output`.
