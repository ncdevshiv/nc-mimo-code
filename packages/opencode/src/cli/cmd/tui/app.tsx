...
function rendererConfig(_config: TuiConfig.Info, plainTerminal: boolean): CliRendererConfig {
  const mouseEnabled = !plainTerminal && !Flag.NC_MIMO_CODE_DISABLE_MOUSE && (_config.mouse ?? true)
...
function App(props: { onSnapshot?: () => Promise<string[]> }) {
  ...
  useKeyboard((evt) => {
    if (!Flag.NC_MIMO_CODE_EXPERIMENTAL_DISABLE_COPY_ON_SELECT) return
  ...
  command.register(() => [
    ...
    },
    {
      ...
      value: "workflow.list",
      category: "session",
      enabled: Flag.NC_MIMO_CODE_EXPERIMENTAL_WORKFLOW_TOOL,
      slash: {
        name: "workflows",
      },
      onSelect: () => {
  ...
  return (
    <box
      ...
      onMouseDown={(evt) => {
        ...
        if (
          Flag.NC_MIMO_CODE_EXPERIMENTAL_DISABLE_COPY_ON_SELECT &&
    ...
    >
      <Show when={Flag.NC_MIMO_CODE_SHOW_TTFD}>
        <TimeToFirstDraw />
      </Show>
      <Show when={ready()}>
        <Switch>
