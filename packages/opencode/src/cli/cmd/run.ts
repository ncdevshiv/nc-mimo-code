...
import { createOpencodeClient, type OpencodeClient, type ToolPart } from "@nc-mimo-code/sdk/v2"
...
export const RunCommand = cmd({
  ...
  handler: async (args) => {
    ...
    async function share(sdk: OpencodeClient, sessionID: string) {
      ...
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.NC_MIMO_CODE_AUTO_SHARE && !args.share) return
    ...
    if (args.attach) {
      const headers = (() => {
        ...
        const username = process.env.NC_MIMO_CODE_SERVER_USERNAME ?? "mimocode"
