...
const APP = "ncmimocode"

export type ResolvedPaths = {
  mode: "nc_mimo_code_home" | "xdg"
  root?: string
  data: string
  cache: string
  config: string
...
/**
 * Resolve nc-mimo-code's four base directories (config/data/state/cache)
 * from environment variables.
 *
 * If NC_MIMO_CODE_HOME is set and non-empty, the four paths are subdirectories
 * of it. Otherwise, falls through to XDG Base Directory defaults.
 *
 * @throws if NC_MIMO_CODE_HOME is set but not an absolute path
 */
export function resolveNcMimoCodeHome(env: NodeJS.ProcessEnv = process.env): ResolvedPaths {
  const home = env.NC_MIMO_CODE_HOME
  if (home) {
    if (!path.isAbsolute(home)) {
      throw new Error(
        `NC_MIMO_CODE_HOME must be an absolute path, got: ${JSON.stringify(home)}`,
      )
    }
    return {
      mode: "nc_mimo_code_home",
      root: home,
...
export namespace Global {
  ...
  export const layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const { data, cache, config, state } = yield* Effect.sync(() => resolveNcMimoCodeHome())
